...
bool KalmanFilter::init(const Matrix12d& u_cov,
  ...
  if (u_sigmas(11) < 1e-10) {
    is_valid = false;
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Input Cov is close to singular (least singlar value:%f < 1e-7)",
        u_sigmas(11));
  } else {
    input_cov = u_cov;
  }
  ...
  if (m_sigmas(5) < 1e-10) {
    is_valid = false;
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Measurement Cov is close to singular (least singlar value:%f < 1e-7)",
        m_sigmas(5));
  } else {
  ...
  if (freq < 0) {
    is_valid = false;
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Frequency is negative");
  } else {
